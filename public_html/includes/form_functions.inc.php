...
function create_form_input($name, $type, $errors, $values = 'POST', $extras = '') {
	...
	if ($values == 'SESSION') {
		...
		if(isset($extras['value'])) $value = $extras['value'];
	}


	// Conditional to determine what kind of element to create:
	if ( ($type == 'text') || ($type == 'password') || $type == 'number' || $type == 'email') { // Create text or password inputs.
		...
		} elseif($name == 'storage-category' || $name == 'size' || $name == 'other-categories' || $name == 'usb2_3' || $name == 'scsi_ide'){
			$data = $extras['data'];
		} 
