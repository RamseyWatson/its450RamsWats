...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($billing_errors)) { // If everything's OK...
		...
		if (isset($_SESSION['order_id'])) { // Use existing order info:
			...
			$r = mysqli_query($dbc, "CALL update_order({$_SESSION['order_id']},'$uid',{$_SESSION['delivery_address']}, {$_SESSION['shipping']}, $cc_last_four, @total)");
			// Confirm that it worked:
			if ($r) {

				// Retrieve the order ID and total:
				$r = mysqli_query($dbc, 'SELECT @total');
				if (mysqli_num_rows($r) == 1) {
					list($order_total) = mysqli_fetch_array($r);
			...
			// Call the stored procedure:
			$r = mysqli_query($dbc, "CALL add_order({$_SESSION['customer_id']}, '$uid',{$_SESSION['delivery_address']}, {$_SESSION['shipping']}, $cc_last_four, @total, @oid)");
			// Confirm that it worked:
			if ($r) {
